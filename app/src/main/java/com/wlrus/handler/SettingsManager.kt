...
import com.wlrus.AppConfig.ANG_PACKAGE
import com.wlrus.AppConfig.DEFAULT_SUBSCRIPTION_ID
import com.wlrus.AppConfig.GEOIP_PRIVATE
import com.wlrus.AppConfig.GEOSITE_PRIVATE
import com.wlrus.AppConfig.TAG_DIRECT
import com.wlrus.AppConfig.VPN
import com.wlrus.BuiltinSubscriptions
import com.wlrus.dto.ProfileItem
import com.wlrus.dto.RulesetItem
import com.wlrus.dto.SubscriptionItem
import com.wlrus.dto.V2rayConfig
import com.wlrus.enums.EConfigType
import com.wlrus.enums.Language
import com.wlrus.enums.RoutingType
import com.wlrus.enums.VpnInterfaceAddressConfig
import com.wlrus.handler.MmkvManager.decodeAllServerList
import com.wlrus.handler.MmkvManager.decodeServerConfig
import com.wlrus.handler.MmkvManager.decodeSubsList
import com.wlrus.handler.MmkvManager.decodeSubscription
import com.wlrus.handler.MmkvManager.encodeSubscription
...
object SettingsManager {
	...
	private fun BuiltinSubscriptions.BuiltinSub.toSubscriptionItem(): SubscriptionItem {
 	   return SubscriptionItem(
	        remarks = this.remarks,
 	       url = this.url,
	        enabled = this.enabled,
	        // Add other fields that SubscriptionItem has and BuiltinSub provides.
   	     // If BuiltinSub doesn't have these, use defaults:
  	      // lastUpdated = null,
  	      // autoUpdate = false,
   	     // etc.
 	   )
    ...
    if (builtinSubs.isEmpty()) return

    val existingSubs = MmkvManager.decodeSubscriptions()

    for (builtinSub in builtinSubs) {
        if (builtinSub.url.isBlank()) continue

        val existingSub = existingSubs.find { it.subscription.url == builtinSub.url }
        
        if (existingSub == null) {
            // ✅ Convert BuiltinSub → SubscriptionItem
            val subscriptionItem = builtinSub.toSubscriptionItem()
            val key = Utils.getUuid()
            MmkvManager.encodeSubscription(key, subscriptionItem)
            Log.i(AppConfig.TAG, "Added built-in subscription: ${builtinSub.remarks}")
        } else if (BuiltinSubscriptions.FORCE_UPDATE_EXISTING) {
            // ✅ Convert and preserve lastUpdated from existing
            val subscriptionItem = builtinSub.toSubscriptionItem().apply {
                lastUpdated = existingSub.subscription.lastUpdated
            }
            MmkvManager.encodeSubscription(existingSub.guid, subscriptionItem)
            Log.i(AppConfig.TAG, "Updated built-in subscription: ${builtinSub.remarks}")
        }
    }
}
     ...
     */
    fun getVpnMtu(): Int {
        return Utils.parseInt(MmkvManager.decodeSettingsString(AppConfig.PREF_VPN_MTU), AppConfig.VPN_MTU.toIntOrNull() ?: 9000)
    }
